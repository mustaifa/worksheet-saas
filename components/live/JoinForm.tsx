...
export default function JoinForm() {
  ...
  async function join(e: React.FormEvent) {
    ...
    setError("");
    try {
      const res = await fetch(`/api/live/${code.trim().toUpperCase()}/join`, {
        method: "POST",
      ...
      sessionStorage.setItem(`live-participant-${code.trim().toUpperCase()}`, data.participantId);
  ...
  return (
    <div className="max-w-sm mx-auto text-center">
      ...
      <h1 className="text-2xl font-bold text-slate-900 dark:text-white">Join a Live Quiz</h1>
      <form onSubmit={join} className="mt-6 space-y-3 text-left">
        <div>
          <label className="text-xs font-semibold uppercase tracking-wide text-slate-500 dark:text-slate-400">Game Code</label>
          <input
